...
test("3.0.2 repeats 3.0.1 because the negotiated lane Pi consumes is unchanged", () => {
	// v3.0.2 fixed the install scripts to build from the /v3 module path
	// with no contract change (gentle-ai #4686). Diffing
	// contracts/review-integration/v2 and contracts/review-provider-contract
	// between the v3.0.1 and v3.0.2 tags in the gentle-ai source tree showed
	// zero byte changes, so this row repeats 3.0.1. riskEvidence and hint
	// remain dark because neither is proven to reach Pi's negotiated START
	// path.
	const contract = NATIVE_CLI_CONTRACTS["3.0.2"] as Record<string, boolean>;
	assert.equal(contract.riskEvidence, false);
	assert.equal(contract.hint, false);
...
test("no shipped version key was added beyond the pin bump", () => {
	...
	assert.deepEqual(Object.keys(NATIVE_CLI_CONTRACTS), [...DARK_VERSIONS, "2.2.0", "2.2.1", "2.2.2", "2.2.3", "2.4.0", "2.5.0-rc.3", "2.5.0", "2.6.0", "2.7.0", "2.8.0", "2.8.1", "2.8.2", "2.9.0", "2.9.1", "3.0.0", "3.0.1", "3.0.2"]);
});
