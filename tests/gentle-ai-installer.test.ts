...
// v3.0.2 digests pinned from the published release: archive digests verified
...
const EXPECTED_ASSETS = {
	"darwin/amd64": { name: "gentle-ai_3.0.2_darwin_amd64.tar.gz", sha256: "0ffff82ff0b59ac5ed09c1198db73f6e18741b8c431771c4ee46709b23842f23", binarySha256: "8bf3e65ad184ee6a30553698230918acb679799f3817fe3ac7931cfb6751f435" },
	"darwin/arm64": { name: "gentle-ai_3.0.2_darwin_arm64.tar.gz", sha256: "350fe4018dcf98182d15888d64f8583c5e0df2c36732e76ca4430f3f40b5c612", binarySha256: "006092a6538ecea349ec61caa8daeb4dc0c8f5c79a381179755cf6a2669010e5" },
	"linux/amd64": { name: "gentle-ai_3.0.2_linux_amd64.tar.gz", sha256: "1cd4324951c276d5aa2521d6b7d0a03d1ab458c8a2b39460c84513ff2e11ad3f", binarySha256: "20f037aa0d220464d2f890cc8aa43259be16ea115d7119e4f5c425ff2ce0177b" },
	"linux/arm64": { name: "gentle-ai_3.0.2_linux_arm64.tar.gz", sha256: "454df0918e91d65ad5ddd2d830e95195909587d714956080b55913ec7b25e6a4", binarySha256: "a2d35c06af27f3ed19ed4e2ce6739f3ca0b4cabceb02a6a1fe87dda9f3fcc419" },
} as const;
...
test("release mapping selects only the supported official v3.0.2 assets and pinned digests", () => {
	assert.deepEqual(
	...
	assert.equal(resolveGentleAiReleaseAsset("linux", "x64").name, "gentle-ai_3.0.2_linux_amd64.tar.gz");
	assert.equal(resolveGentleAiReleaseAsset("darwin", "arm64").name, "gentle-ai_3.0.2_darwin_arm64.tar.gz");
	for (const asset of Object.values(GENTLE_AI_RELEASE_ASSETS)) {
		assert.match(asset.url, /^https:\/\/github\.com\/Gentleman-Programming\/gentle-ai\/releases\/download\/v3\.0\.2\//);
	}
});
...
test("raw release assets are admitted only under a prerelease pin", () => {
	...
	assert.throws(() => gentleAiAssetForm("gentle-ai_3.0.2_linux_amd64", "3.0.2"), /only admitted for a prerelease pin/);
...
test("release digests are all-or-none and install fails closed while any digest is pending", async () => {
	...
	if (pending.length === digests.length) {
		...
		assert.equal(existsSync(join(packageRoot, ".gentle-ai", "v3.0.2", "gentle-ai")), false);
	}
});
...
function windowsGoFixture(fixtureOptions: WindowsGoFixtureOptions = {}) {
	...
	const metadata = [
		...
		`\tmod\tgithub.com/gentleman-programming/gentle-ai/v3\tv3.0.2\t${GENTLE_AI_WINDOWS_SOURCE_MODULE_CHECKSUM}`,
	...
	const run = async (file: string, arguments_: string[], options: WindowsGoCall["options"]) => {
		...
		if (arguments_.length === 1 && arguments_[0] === "version") return { stdout: fixtureOptions.reportedVersion ?? "gentle-ai 3.0.2\n", stderr: "" };
...
test("win32 x64 and arm64 install the exact Go SumDB source tag without archive lookup", async () => {
	for (const [arch, goArchitecture] of [["x64", "amd64"], ["arm64", "arm64"]] as const) {
		...
		assert.deepEqual(fixture.calls.filter((call) => call.file === goPath).map((call) => call.arguments_.slice(0, 2)), [
			["version"], ["install", "github.com/gentleman-programming/gentle-ai/v3/cmd/gentle-ai@v3.0.2"], ["version", "-m"],
		]);
	}
});
...
test("Windows source install reports missing or too-old Go without publishing a binary", async () => {
	for (const fixtureOptions of [
		...
		assert.equal(existsSync(join(packageRoot, ".gentle-ai", "v3.0.2", "gentle-ai.exe")), false);
...
test("Windows source install cleans staging after Go failure or wrong built version", async () => {
	for (const fixtureOptions of [
		...
		const runtimeDirectory = join(packageRoot, ".gentle-ai", "v3.0.2");
...
test("Windows source installs reuse only a fully verified package-local binary", async () => {
	...
	await writeFile(join(packageRoot, ".gentle-ai", "v3.0.2", "integrity.json"), "{}\n");
...
async function hardenedWindowsGoFixture(packageRoot: string, fixtureOptions: HardenedGoFixtureOptions = {}) {
	...
	const run = async (file: string, arguments_: string[], options: HardenedGoCall["options"]) => {
		...
		if (arguments_.length === 1 && arguments_[0] === "version") return { stdout: "gentle-ai 3.0.2\n", stderr: "" };
		throw new Error(`unexpected command: ${file} ${arguments_.join(" ")}`);
	};
	return {
		calls,
...
test("Windows source installation resolves one validated Go executable and seals hostile Go environment", async () => {
	...
	assert.deepEqual(goCalls.map((call) => call.arguments_.slice(0, 2)), [
		["version"],
		["install", "github.com/gentleman-programming/gentle-ai/v3/cmd/gentle-ai@v3.0.2"],
		["version", "-m"],
	]);
...
test("Windows source manifest binds verified Go metadata and architecture", async () => {
	...
	const manifest = JSON.parse(await readFile(join(packageRoot, ".gentle-ai", "v3.0.2", "integrity.json"), "utf8")) as Record<string, string>;
	assert.equal(result.installed, true);
...
test("Windows source installation treats a fresh ownerless lock as active", async () => {
	...
	const lockPath = join(packageRoot, ".gentle-ai", ".v3.0.2.install.lock");
...
test("Windows source installation preserves a lock whose owner nonce changed before release", async () => {
	...
	const lockOwnerPath = join(packageRoot, ".gentle-ai", ".v3.0.2.install.lock", "owner.json");
	const fixture = await hardenedWindowsGoFixture(packageRoot);
...
function tombstonePath(packageRoot: string, nonce: string): string {
	return join(packageRoot, ".gentle-ai", `.v3.0.2.install.tombstone-${nonce}`);
}
...
async function tombstones(packageRoot: string): Promise<string[]> {
	...
	return existsSync(runtimeRoot)
		? (await readdir(runtimeRoot)).filter((entry) => entry.startsWith(".v3.0.2.install.tombstone-"))
		: [];
}

function backupBundlePath(packageRoot: string, nonce: string): string {
	return join(packageRoot, ".gentle-ai", `.v3.0.2.backup-${nonce}`);
}
...
test("Windows source acquisition fails closed when a tombstone appears after its first scan", async () => {
	...
	assert.equal(existsSync(join(packageRoot, ".gentle-ai", ".v3.0.2.install.lock")), false);
	assert.equal(fixture.calls.some((call) => call.arguments_[0] === "install"), false);
});
...
test("Windows source recovers a valid backup after a crash between publication renames before any build", async () => {
	...
	const live = join(packageRoot, ".gentle-ai", "v3.0.2");
...
test("Windows source publication rolls back a prior bundle when final directory swap fails", async () => {
	...
	const versionDirectory = join(packageRoot, ".gentle-ai", "v3.0.2");
...
test("Darwin/Linux signed bundles retain their four-field manifest and reusable compatibility", async () => {
	...
	const manifestPath = join(packageRoot, ".gentle-ai", "v3.0.2", "integrity.json");
...
test("a pinned asset falls back to its gentle-pi mirror only on download failure, and a stable pin declares no mirror", async () => {
	...
	const baseAsset = { name: "gentle-ai_3.0.2_linux_amd64.tar.gz", sha256: createHash("sha256").update(payload).digest("hex"), binarySha256: createHash("sha256").update(binary).digest("hex"), url: "https://example.invalid/upstream", mirrorUrl: "https://example.invalid/mirror", executable: "gentle-ai" };
...
test("installer promotes only the expected regular executable with executable POSIX mode", async () => {
	...
	const binary = join(packageRoot, ".gentle-ai", "v3.0.2", "gentle-ai");
